...
private:
  ...
  void initCompression(const int32_t jpegQuality, const int32_t pngLevel, const bool use_png)
  {
    ...
    compressionParams[0] = cv::ImwriteFlags::IMWRITE_JPEG_QUALITY;
    compressionParams[1] = jpegQuality;
    compressionParams[2] = cv::ImwriteFlags::IMWRITE_PNG_COMPRESSION;
    compressionParams[3] = pngLevel;
    compressionParams[4] = cv::ImwriteFlags::IMWRITE_PNG_STRATEGY;
    compressionParams[5] = cv::ImwritePNGFlags::IMWRITE_PNG_STRATEGY_RLE;
  ...
  void receiveColor()
  {
       ...
       status[MONO_HD] || status[MONO_HD_RECT] || status[MONO_QHD] || status[MONO_QHD_RECT])
    {
      ...
      {
        cv::cvtColor(tmp, images[COLOR_HD], cv::ColorConversionCodes::COLOR_BGRA2BGR);
      }
      else
      {
        cv::cvtColor(tmp, images[COLOR_HD], cv::ColorConversionCodes::COLOR_RGBA2BGR);
      }
    }
  ...
  void processIrDepth(const cv::Mat &depth, std::vector<cv::Mat> &images, const std::vector<Status> &status)
  {
    ...
    if(status[COLOR_SD_RECT])
    {
      ...
      {
        cv::cvtColor(tmp, images[COLOR_SD_RECT], cv::ColorConversionCodes::COLOR_BGRA2BGR);
      }
      else
      {
        cv::cvtColor(tmp, images[COLOR_SD_RECT], cv::ColorConversionCodes::COLOR_RGBA2BGR);
      }
    }
  ...
  void processColor(std::vector<cv::Mat> &images, const std::vector<Status> &status)
  {
    ...
    if(status[MONO_HD])
    {
      cv::cvtColor(images[COLOR_HD], images[MONO_HD], cv::ColorConversionCodes::COLOR_BGR2GRAY);
    }
    if(status[MONO_HD_RECT])
    {
      cv::cvtColor(images[COLOR_HD_RECT], images[MONO_HD_RECT], cv::ColorConversionCodes::COLOR_BGR2GRAY);
    }
    if(status[MONO_QHD])
    {
      cv::cvtColor(images[COLOR_QHD], images[MONO_QHD], cv::ColorConversionCodes::COLOR_BGR2GRAY);
    }
    if(status[MONO_QHD_RECT])
    {
      cv::cvtColor(images[COLOR_QHD_RECT], images[MONO_QHD_RECT], cv::ColorConversionCodes::COLOR_BGR2GRAY);
    }
  }
